...
import { View, Text, StyleSheet, FlatList, ActivityIndicator, Platform, RefreshControl, Pressable, Modal, ScrollView, TextInput, Alert, } from "react-native";
...
export default function TripsScreen() {
    ...
    function openTrip(trip) { setSelectedTrip(trip); setHelpSent(false); setHelpMessage(""); }
    ...
    const renderTrip = useCallback(({ item }) => {
        ...
        return (<Pressable style={({ pressed }) => [styles.tripCard, isActive && styles.tripCardActive, pressed && { opacity: 0.85 }]} onPress={() => openTrip(item)}>
        ...
        <View style={styles.tripFooter}>
          <Text style={styles.vehicleType}>{(item.vehicleType || "Standard").replace(/\b\w/g, (c) => c.toUpperCase())}</Text>
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]}>
      <Text style={styles.title}>Trip History</Text>
      {isLoading ? (<View style={styles.center}><ActivityIndicator size="large" color={Colors.white}/></View>) : !trips || !trips.length ? (<View style={styles.center}>
        ...
        </View>) : (<FlatList data={trips} keyExtractor={(item) => String(item.id)} renderItem={renderTrip} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={Colors.white}/>}/>)}
      ...
      <Modal visible={!!selectedTrip} animationType="slide" presentationStyle="overFullScreen" transparent={false} onRequestClose={closeTrip}>
        <View style={styles.modalContainer}>
          ...
          {selectedTrip && (<ScrollView contentContainerStyle={styles.modalContent} showsVerticalScrollIndicator={false}>
              {(() => {
                const { label, color } = getStatusConfig(selectedTrip.status);
                return (<View style={[styles.detailStatusChip, { backgroundColor: `${color}22` }]}>
                    <View style={[styles.statusDot, { backgroundColor: color }]}/>
              ...
              <View style={styles.fareCard}>
                ...
                {selectedTrip.distanceKm && (<View style={styles.fareRow}>
              ...
              <View style={styles.helpSection}>
                ...
                {helpSent ? (<View style={styles.helpSentBox}>
                    <Ionicons name="checkmark-circle" size={24} color={Colors.success}/>
                    <Text style={styles.helpSentText}>Message sent! We'll get back to you shortly.</Text>
                  </View>) : (<>
                    <TextInput style={styles.helpInput} placeholder="e.g. I was overcharged, driver was late..." placeholderTextColor={Colors.textMuted} value={helpMessage} onChangeText={setHelpMessage} multiline numberOfLines={4} textAlignVertical="top"/>
                    <Pressable style={({ pressed }) => [styles.helpSendBtn, (pressed || sendEnquiry.isPending) && { opacity: 0.7 }]} onPress={() => {
                    if (!helpMessage.trim()) {
