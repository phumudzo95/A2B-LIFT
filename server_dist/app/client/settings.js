    ...
    return (<ScrollView style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      <View style={styles.headerRow}>
        ...
        <Pressable onPress={() => editMode ? handleSave() : setEditMode(true)}>
          {saving ? <ActivityIndicator size="small" color={Colors.white}/> : (<Ionicons name={editMode ? "checkmark" : "create-outline"} size={22} color={Colors.white}/>)}
        </Pressable>
      </View>
      ...
      <View style={styles.card}>
        <View style={styles.settingRow}>
          ...
          {editMode ? (<TextInput style={styles.settingInput} value={name} onChangeText={setName}/>) : (<Text style={styles.settingValue}>{user?.name || "—"}</Text>)}
        </View>
        ...
        <View style={styles.settingRow}>
          ...
          {editMode ? (<TextInput style={styles.settingInput} value={phone} onChangeText={setPhone} keyboardType="phone-pad"/>) : (<Text style={styles.settingValue}>{user?.phone || "Not set"}</Text>)}
        </View>
      </View>
...
const styles = StyleSheet.create({
    ...
    settingInput: { fontSize: 14, fontFamily: "Inter_400Regular", color: Colors.white, backgroundColor: Colors.surface, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, minWidth: 140, textAlign: "right" },
