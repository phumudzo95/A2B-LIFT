...
export default function NotificationsScreen() {
    ...
    const renderNotification = useCallback(({ item }) => (<Pressable style={[styles.notifCard, !item.isRead && styles.notifUnread]} onPress={() => { if (!item.isRead)
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]}>
      ...
      {isLoading ? (<View style={styles.center}>
        ...
        </View>) : !notifications || (Array.isArray(notifications) && notifications.length === 0) ? (<View style={styles.center}>
          <Ionicons name="notifications-off-outline" size={48} color={Colors.textMuted}/>
          <Text style={styles.emptyText}>No notifications</Text>
          <Text style={styles.emptySubtext}>You're all caught up</Text>
        </View>) : (<FlatList data={Array.isArray(notifications) ? notifications : []} keyExtractor={(item) => item.id} renderItem={renderNotification} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={false} onRefresh={refetch} tintColor={Colors.white}/>}/>)}
    </View>);
}
