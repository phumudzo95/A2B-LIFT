...
import { View, Text, StyleSheet, Pressable, TextInput, ActivityIndicator, Platform, Modal, FlatList, Alert, Linking, ScrollView, Image, KeyboardAvoidingView, } from "react-native";
...
import { getBestAvailablePosition, isRecentLocation, toLatLng, watchBestPosition, } from "@/lib/location-utils";
import { useSocket } from "@/lib/socket-context";
import { uploadDocument } from "@/lib/supabase-storage";
import Colors from "@/constants/colors";
import A2BMap from "@/components/A2BMap";
const VEHICLE_TYPES = [
    { id: "budget", name: "Budget", desc: "Toyota Corolla, Toyota Quest", icon: "car-outline", pricePerKm: 7, baseFare: 50 },
    { id: "luxury", name: "Luxury", desc: "BMW 3 Series, Mercedes C Class", icon: "car-sport", pricePerKm: 13, baseFare: 100 },
    { id: "business", name: "Business Class", desc: "BMW 5 Series, Mercedes E Class", icon: "briefcase", pricePerKm: 35, baseFare: 150 },
    { id: "van", name: "Van", desc: "Hyundai H1, Mercedes Vito, Staria", icon: "bus", pricePerKm: 13, baseFare: 120 },
    { id: "luxury_van", name: "Luxury Van", desc: "Mercedes V Class", icon: "car", pricePerKm: 35, baseFare: 200 },
];
function getRoutePreferenceLabel(routeId) {
    if (routeId === "faster_route")
        return "Faster Route";
    if (routeId === "safest_route")
        return "Safer Route";
...
function dedupeDirectionRoutes(routes) {
    const seen = new Set();
    const unique = [];
    for (const route of routes) {
        if (!route?.polyline || seen.has(route.polyline))
            continue;
...
function buildRouteChoiceDescriptors(routes) {
...
function calculateFallbackEstimate(distanceKm, vehicle, isLateNight) {
...
function hasLocationShift(previous, next, minimumDistanceKm = SIGNIFICANT_LOCATION_SHIFT_KM) {
    if (!previous || !next)
        return true;
    return haversineDistance(previous.lat, previous.lng, next.lat, next.lng) >= minimumDistanceKm;
}
function formatNativeReverseGeocode(address) {
    if (!address)
        return null;
...
async function buildNativeLocationSuggestions(query) {
    try {
        const results = await Location.geocodeAsync(query);
        const uniqueResults = results.filter((result, index, all) => {
            return all.findIndex((candidate) => Math.abs(candidate.latitude - result.latitude) < 0.0001 &&
                Math.abs(candidate.longitude - result.longitude) < 0.0001) === index;
        }).slice(0, 5);
        const suggestions = await Promise.all(uniqueResults.map(async (result, index) => {
...
function mergeNearbyDrivers(current, incoming) {
...
export default function ClientHomeScreen() {
    ...
    const [lateNightPremium, setLateNightPremium] = useState(0);
    const [routeChoices, setRouteChoices] = useState([]);
    const [selectedRouteId, setSelectedRouteId] = useState(null);
    const [currentRide, setCurrentRide] = useState(null);
    const [showVehicleSheet, setShowVehicleSheet] = useState(false);
    const [chauffeurDetails, setChauffeurDetails] = useState(null);
    const [routePolyline, setRoutePolyline] = useState(null);
    const [tripDurationText, setTripDurationText] = useState(null);
    ...
    const [livenessSelfieLocalUri, setLivenessSelfieLocalUri] = useState(null);
    const [livenessSelfieUrl, setLivenessSelfieUrl] = useState(null);
    ...
    const [liveEtaMin, setLiveEtaMin] = useState(null);
    const [initialEtaMin, setInitialEtaMin] = useState(null);
    // ETA to nearest available driver (shown on map in idle/selecting state)
    const [nearestDriverEta, setNearestDriverEta] = useState(null);
    // Location picker modal
    const [locationPickerVisible, setLocationPickerVisible] = useState(false);
    const [locationPickerTarget, setLocationPickerTarget] = useState("dropoff");
    ...
    const lastResolvedPickupRef = useRef(null);
        ...
        async function fetchOnlineDrivers() {
            try {
                ...
                const online = all
                    ...
                    .map((c) => ({ id: String(c.id), lat: Number(c.lat), lng: Number(c.lng) }));
    ...
    useEffect(() => {
        const handleNearbyDriverLocation = (data) => {
            if (!data?.chauffeurId || data.lat == null || data.lng == null)
                return;
            const nextDriver = {
    ...
    async function fetchChauffeurDetails(chauffeurId) {
        try {
    ...
    async function resolvePickupAddress(coords, force = false) {
        if (!force && !pickupFollowsDeviceRef.current)
            return;
        if (!force && !hasLocationShift(lastResolvedPickupRef.current, coords, 0.08))
            return;
    ...
    async function startLocationWatch() {
        if (Platform.OS === "web" || locationWatchRef.current)
            return;
        try {
    ...
    const applyRideUpdate = useCallback((ride) => {
        if (ride.status === "cancelled") {
            ...
            Alert.alert("Ride Cancelled", "Your ride has been cancelled by the driver. Please request a new ride.", [{ text: "OK" }]);
            return;
        }
        setCurrentRide(ride);
        if (ride.status === "chauffeur_assigned") {
            ...
            if (ride.chauffeurId) {
                ...
                apiRequest("GET", `/api/chauffeurs/${ride.chauffeurId}`).then(r => r.json()).then((c) => {
            ...
            if (ride.dropoffLat && ride.dropoffLng) {
                setDriverLocation((prev) => {
                    if (prev)
                        fetchRoute(prev, { lat: ride.dropoffLat, lng: ride.dropoffLng });
                    return prev;
                });
            }
        }
    ...
    useEffect(() => {
        if (rideStatus !== "requested" || !currentRide?.id)
            return;
    ...
    useEffect(() => {
        ...
        if (!activeStatuses.includes(rideStatus) || !currentRide?.id)
            return;
        const pollId = setInterval(async () => {
            try {
                const res = await apiRequest("GET", `/api/rides/${currentRideRef.current?.id}`);
    ...
    useEffect(() => {
        if (rideStatus !== "assigned" && rideStatus !== "arriving")
            return;
    ...
    async function fetchRoute(origin, dest) {
        try {
            const res = await apiRequest("GET", `/api/directions?originLat=${origin.lat}&originLng=${origin.lng}&destLat=${dest.lat}&destLng=${dest.lng}`);
            const data = await res.json();
            if (data.polyline) {
                setRoutePolyline(data.polyline);
                if (data.durationText)
                    setTripDurationText(data.durationText);
                if (data.durationMin) {
                ...
                if (data.distanceKm)
                    setEstimatedDistance(Math.round(data.distanceKm * 10) / 10);
                setEtaText(`ETA: ${data.durationText}`);
            }
        }
        catch { }
    }
    function applyRouteChoice(choice) {
    ...
    async function fetchRouteChoices(origin, dest) {
        const res = await apiRequest("GET", `/api/directions?originLat=${origin.lat}&originLng=${origin.lng}&destLat=${dest.lat}&destLng=${dest.lng}`);
        const data = await res.json();
        ...
        const choices = await Promise.all(choiceDescriptors.map(async ({ id, route, title, subtitle, badge, icon }) => {
    ...
    async function requestRide() {
        if (!user || !location || !dropoffCoords)
            return;
    ...
    async function createRideRecord(method, extras = {}) {
        if (!user || !location || !dropoffCoords)
            return null;
    ...
    async function handlePayAndRide(method) {
        if (!user || !location || !dropoffCoords)
            return;
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      ...
      <View style={styles.mapArea}>
        <A2BMap pickupLocation={location} dropoffLocation={dropoffCoords} driverLocation={driverLocation} nearbyDrivers={onlineDrivers} routePolyline={routePolyline} showDriver={rideStatus === "assigned" || rideStatus === "arriving" || rideStatus === "in_trip"} followDriver={rideStatus === "arriving" || rideStatus === "in_trip"} loading={locationLoading} etaText={etaText || undefined} statusText={rideStatus === "in_trip" ? "Trip In Progress" : undefined}/>

        {/* Nearest driver ETA pill — shown when idle and drivers are nearby */}
        {(rideStatus === "idle" || rideStatus === "selecting") && nearestDriverEta && onlineDrivers.length > 0 && (<View style={styles.nearbyEtaPill}>
        ...
        {routePolyline && (rideStatus === "selecting" || rideStatus === "confirming") && (estimatedDistance || tripDurationText) && (() => {
            ...
            const arrivalStr = arrivalTime
                ...
                : null;
            return (<View style={styles.routeInfoOverlay}>
              {arrivalStr && (<View style={styles.arrivalPill}>
                  <Ionicons name="time-outline" size={13} color="#fff"/>
                  <Text style={styles.arrivalPillText}>Arrive by {arrivalStr}</Text>
                </View>)}
              {(estimatedDistance || tripDurationText) && (<View style={styles.routeMetaPill}>
        ...
        {rideStatus === "requested" && (<View style={styles.searchingMapOverlay}>
        ...
        {(rideStatus === "assigned" || rideStatus === "arriving") && chauffeurDetails && (<Animated.View entering={FadeInDown.duration(500)} style={styles.liveNotifBanner}>
      ...
      {rideStatus === "idle" && (<Animated.View entering={FadeInDown.duration(400)} style={[styles.bottomSheet, { marginBottom: bottomPanelOffset, paddingBottom: bottomPanelPadding }]}>
          ...
          <View style={styles.locationInputsCard}>
            ...
            <Pressable style={styles.locationInputRow} onPress={() => openLocationPicker("dropoff")}>
              ...
              <View style={styles.locationInputInner}>
                ...
                <Text style={[styles.locationInputValue, !dropoffAddress && { color: Colors.textMuted }]} numberOfLines={1}>
          ...
          <Pressable style={styles.vehicleSelector} onPress={() => setShowVehicleSheet(true)}>
          ...
          <Pressable style={({ pressed }) => [styles.confirmBtn, pressed && { opacity: 0.9, transform: [{ scale: 0.98 }] }]} onPress={getEstimate}>
            <Text style={styles.confirmBtnText}>Get Estimated Fare</Text>
          </Pressable>
        </Animated.View>)}

      {rideStatus === "confirming" && (<Animated.View entering={FadeInDown.duration(400)} style={[styles.confirmingSheet, { marginBottom: bottomPanelOffset }]}>
          <View style={styles.sheetHandle}/>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[
            ...
            <View style={styles.priceCard}>
              ...
              {(estimatedDistance || tripDurationText) && (<View style={styles.tripInfoPill}>
                  {estimatedDistance && (<Text style={styles.tripInfoText}>{estimatedDistance} km</Text>)}
                  {estimatedDistance && tripDurationText && (<Text style={styles.tripInfoSep}>·</Text>)}
                  {tripDurationText && (<Text style={styles.tripInfoText}>{tripDurationText}</Text>)}
                </View>)}
              {selectedRouteChoice ? (<View style={styles.estimateBadgeRow}>
            ...
            {routeChoices.length > 0 && (<View style={styles.routeChoiceSection}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.routeChoiceScroller}>
                  {routeChoices.map((choice) => {
                    ...
                    return (<Pressable key={choice.id} style={[styles.routeChoiceCard, isSelected && styles.routeChoiceCardSelected]} onPress={() => applyRouteChoice(choice)}>
            ...
            <View style={styles.fareBreakdown}>
              ...
              {lateNightPremium > 0 && (<View style={styles.fareRow}>
            ...
            <Pressable style={({ pressed }) => [styles.requestBtn, pressed && { opacity: 0.9 }]} onPress={requestRide}>
      ...
      {rideStatus === "requested" && (<Animated.View entering={FadeInDown.duration(400)} style={[styles.searchingBottomSheet, { marginBottom: bottomPanelOffset, paddingBottom: bottomPanelPadding }]}>
          <View style={styles.sheetHandle}/>
      ...
      {rideStatus === "no_drivers" && (<Animated.View entering={FadeInDown.duration(400)} style={[styles.bottomSheet, { marginBottom: bottomPanelOffset, paddingBottom: bottomPanelPadding }]}>
          <View style={styles.sheetHandle}/>
      ...
      {(rideStatus === "assigned" || rideStatus === "arriving" || rideStatus === "in_trip") && (<Animated.View entering={FadeInDown.duration(400)} style={[styles.bottomSheet, { marginBottom: bottomPanelOffset, paddingBottom: bottomPanelPadding }]}>
          <View style={styles.sheetHandle}/>
          ...
          <View style={styles.chauffeurCard}>
            <Pressable style={styles.chauffeurAvatarBtn} onPress={openDriverProfile}>
              <View style={styles.chauffeurAvatar}>
                {chauffeurDetails?.profilePhoto ? (<Image source={{ uri: chauffeurDetails.profilePhoto }} style={{ width: 48, height: 48, borderRadius: 24 }} resizeMode="cover"/>) : (<Ionicons name="person" size={24} color={Colors.white}/>)}
              </View>
            ...
            <Pressable style={styles.chauffeurInfo} onPress={openDriverProfile}>
              ...
              {chauffeurDetails && (<View style={styles.driverMeta}>
                  ...
                  {/* Car color dot */}
                  {chauffeurDetails.carColor ? (<View style={styles.colorBadge}>
          ...
          {currentRide?.price && (<View style={styles.tripPriceRow}>
          ...
          <View style={styles.selectionMetaRow}>
            ...
            <View style={styles.selectionMetaChip}>
              <Ionicons name={currentRide?.paymentMethod === "card" ? "card-outline" : currentRide?.paymentMethod === "wallet" ? "wallet-outline" : "cash-outline"} size={14} color={Colors.white}/>
              <Text style={styles.selectionMetaText}>{getPaymentMethodLabel(currentRide?.paymentMethod)}</Text>
            </View>
          </View>
          {(rideStatus === "assigned" || rideStatus === "arriving" || rideStatus === "in_trip") && (<Pressable style={styles.cancelRideActiveBtn} onPress={() => {
                    if (Platform.OS === "web") {
                        if (global.confirm?.("Are you sure you want to cancel this ride?") !== false) {
                            cancelRide();
                        }
                    }
                    else {
      ...
      {rideStatus === "completed" && !showRating && (<Animated.View entering={FadeInDown.duration(400)} style={[styles.bottomSheet, { marginBottom: bottomPanelOffset, paddingBottom: bottomPanelPadding }]}>
          <View style={styles.sheetHandle}/>
          ...
          <Pressable style={({ pressed }) => [styles.confirmBtn, pressed && { opacity: 0.9 }]} onPress={() => setShowRating(true)}>
            <Text style={styles.confirmBtnText}>Rate Your Driver</Text>
          </Pressable>
        </Animated.View>)}

      {showRating && (<KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : 20} style={styles.ratingSheetKeyboard}>
          <View style={styles.ratingSheetOverlay}>
            <ScrollView bounces={false} contentContainerStyle={[styles.ratingSheetScrollContent, { paddingBottom: bottomPanelOffset }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              <Animated.View entering={FadeInDown.duration(400)} style={[styles.bottomSheet, styles.ratingSheetCard, { paddingBottom: bottomPanelPadding }]}> 
                ...
                <View style={styles.ratingContainer}>
                  ...
                  <View style={styles.starsContainer}>
                    {[1, 2, 3, 4, 5].map((star) => (<Pressable key={star} onPress={() => setRating(star)} style={({ pressed }) => [styles.starButton, pressed && { opacity: 0.7 }]}>
                        <Ionicons name={star <= rating ? "star" : "star-outline"} size={40} color={star <= rating ? Colors.warning : Colors.textMuted}/>
                      </Pressable>))}
                  </View>
                </View>
                ...
                <View style={styles.commentContainer}>
                  ...
                  <TextInput style={styles.commentInput} placeholder="Share your experience..." placeholderTextColor={Colors.textMuted} value={ratingComment} onChangeText={setRatingComment} multiline numberOfLines={3} textAlignVertical="top" scrollEnabled={false}/>
                </View>

                <View style={[styles.ratingActions, { paddingBottom: insets.bottom + 8 }]}> 
                  <Pressable style={({ pressed }) => [styles.skipButton, pressed && { opacity: 0.8 }]} onPress={() => {
                setShowRating(false);
                resetAfterComplete();
            }}>
                ...
                pressed && { opacity: 0.9 },
            ]} onPress={submitRating} disabled={rating === 0 || submittingRating}>
                    {submittingRating ? (<ActivityIndicator size="small" color={Colors.white}/>) : (<Text style={styles.submitRatingButtonText}>Submit</Text>)}
                  </Pressable>
                </View>
              </Animated.View>
            </ScrollView>
      ...
      <Modal visible={showDriverProfile} transparent animationType="slide" onRequestClose={() => setShowDriverProfile(false)}>
        <View style={styles.profileModalOverlay}>
          <View style={[styles.profileModalSheet, { paddingBottom: insets.bottom + 16 }]}>
            ...
            {driverProfileLoading ? (<View style={styles.profileLoadingContainer}>
                <ActivityIndicator size="large" color={Colors.primary}/>
                <Text style={styles.profileLoadingText}>Loading profile...</Text>
              </View>) : driverProfile ? (<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
                {/* Profile Header */}
                <View style={styles.profileHero}>
                  <View style={styles.profileAvatarLarge}>
                    {driverProfile.profilePhoto ? (<Image source={{ uri: driverProfile.profilePhoto }} style={styles.profileAvatarImg} resizeMode="cover"/>) : (<Ionicons name="person" size={44} color={Colors.white}/>)}
                  </View>
                  ...
                  {driverProfile.plateNumber ? (<View style={styles.profilePlateChip}>
                      <Text style={styles.profilePlateText}>{driverProfile.plateNumber}</Text>
                    </View>) : null}
                  <View style={styles.profileHighlights}>
                    {driverProfile.vehicleCategory ? (<View style={styles.profileHighlightChip}>
                    ...
                    {driverProfile.carColor ? (<View style={styles.profileHighlightChip}>
                ...
                <View style={styles.profileStatsRow}>
                  <View style={styles.profileStatBox}>
                    ...
                    <View style={{ flexDirection: "row", gap: 2, justifyContent: "center", marginBottom: 2 }}>
                      {[1, 2, 3, 4, 5].map((s) => (<Ionicons key={s} name={s <= Math.round(driverProfile.driverRating ?? 0) ? "star" : "star-outline"} size={11} color={Colors.warning}/>))}
                    </View>
                ...
                {driverProfile.totalRatings > 0 && (<View style={styles.profileDistribution}>
                    ...
                    return (<View key={star} style={styles.distRow}>
                ...
                {driverProfile.ratings.length > 0 ? (<View style={styles.profileReviews}>
                    <Text style={styles.profileSectionTitle}>Recent Reviews</Text>
                    {driverProfile.ratings.map((review) => (<View key={review.id} style={styles.reviewCard}>
                        <View style={styles.reviewHeader}>
                          ...
                          <View style={styles.reviewStars}>
                            {[1, 2, 3, 4, 5].map((s) => (<Ionicons key={s} name={s <= review.rating ? "star" : "star-outline"} size={12} color={Colors.warning}/>))}
                          </View>
                        </View>
                        {review.comment ? (<Text style={styles.reviewComment}>{review.comment}</Text>) : null}
                      </View>))}
                  </View>) : (<View style={styles.noReviewsContainer}>
      ...
      <Modal visible={showPaymentPicker} transparent animationType="slide" onRequestClose={() => setShowPaymentPicker(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowPaymentPicker(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]}>
            ...
            {selectedRouteChoice ? (<View style={styles.paymentContextRow}>
            ...
            return (<Pressable style={styles.payMethodRow} onPress={() => {
                    if (!defaultCard) {
                        setShowPaymentPicker(false);
            ...
            {(user?.walletBalance || 0) >= (estimatedPrice || 0) && (estimatedPrice || 0) > 0 && (<Pressable style={styles.payMethodRow} onPress={() => handlePayAndRide("wallet")}>
      ...
      <Modal visible={showCashLiveness} animationType="slide" onRequestClose={() => resetCashLiveness(true)}>
        <ScrollView style={{ flex: 1, backgroundColor: Colors.primary }} contentContainerStyle={[styles.livenessContainer, { paddingTop: insets.top + 8, flexGrow: 1 }]} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.livenessCard}>
            ...
            {!!livenessChallenge && (<View style={styles.challengeBox}>
          ...
          <View style={styles.livenessPreviewWrap}>
            {livenessSelfieLocalUri ? (<Image source={{ uri: livenessSelfieLocalUri }} style={styles.livenessPreviewImg}/>) : (<View style={styles.livenessPlaceholder}>
          ...
          {!!livenessMessage && (<Text style={styles.livenessStatusText}>{livenessMessage}</Text>)}

          <View style={styles.livenessActions}>
            <Pressable style={[styles.livenessBtnSecondary, livenessBusy && { opacity: 0.6 }]} disabled={livenessBusy} onPress={captureAndVerifyLiveness}>
              {livenessBusy ? (<ActivityIndicator size="small" color={Colors.white}/>) : (<Text style={styles.livenessBtnSecondaryText}>
                  {livenessSelfieLocalUri ? "Retake Selfie" : "Start Liveness"}
                </Text>)}
            </Pressable>

            <Pressable style={[styles.livenessBtnPrimary, !livenessPassed && styles.livenessBtnDisabled]} disabled={!livenessPassed || livenessBusy} onPress={continueWithVerifiedCashRide}>
      ...
      <Modal visible={locationPickerVisible} animationType="slide" onRequestClose={() => setLocationPickerVisible(false)}>
        <View style={[styles.locationPickerContainer, { paddingTop: insets.top }]}>
          ...
          <View style={styles.locationPickerInputRow}>
            ...
            <TextInput style={styles.locationPickerInput} placeholder={locationPickerTarget === "pickup" ? "Search pickup location..." : "Search destination..."} placeholderTextColor={Colors.textMuted} value={locationPickerQuery} onChangeText={onLocationQueryChange} autoFocus returnKeyType="search" clearButtonMode="while-editing"/>
            {suggestionsLoading && <ActivityIndicator size="small" color={Colors.textMuted}/>}
          </View>

          {/* Use current location (pickup only) */}
          {locationPickerTarget === "pickup" && (<Pressable style={styles.currentLocationBtn} onPress={useCurrentLocationForPickup}>
              <Ionicons name="locate" size={18} color={Colors.white}/>
              <Text style={styles.currentLocationText}>Use my current location</Text>
            </Pressable>)}

          {/* Suggestions list */}
          <FlatList data={locationSuggestions} keyExtractor={(item) => item.placeId} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: insets.bottom + 20 }} ItemSeparatorComponent={() => <View style={styles.suggestionDivider}/>} ListEmptyComponent={locationPickerQuery.length >= 2 && !suggestionsLoading ? (<View style={styles.noSuggestionsContainer}>
                  <Text style={styles.noSuggestionsText}>No results found</Text>
                </View>) : null} renderItem={({ item }) => (<Pressable style={({ pressed }) => [styles.suggestionRow, pressed && { backgroundColor: Colors.surface }]} onPress={() => selectSuggestion(item)}>
                <View style={styles.suggestionIcon}>
                  <Ionicons name="location-outline" size={18} color={Colors.textSecondary}/>
                </View>
                <View style={styles.suggestionText}>
                  <Text style={styles.suggestionMain} numberOfLines={1}>{item.mainText}</Text>
                  {item.secondaryText ? (<Text style={styles.suggestionSecondary} numberOfLines={1}>{item.secondaryText}</Text>) : null}
                </View>
              </Pressable>)}/>
        </View>
      </Modal>
      ...
      <Modal visible={showVehicleSheet} transparent animationType="slide" onRequestClose={() => setShowVehicleSheet(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowVehicleSheet(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]}>
            ...
            {VEHICLE_TYPES.map((vt) => (<Pressable key={vt.id} style={({ pressed }) => [
                ...
                {selectedVehicle.id === vt.id && (<Ionicons name="checkmark-circle" size={22} color={Colors.white}/>)}
              </Pressable>))}
          </View>
        </Pressable>
      </Modal>
