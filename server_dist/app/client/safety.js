...
import { View, Text, StyleSheet, Pressable, TextInput, ActivityIndicator, Platform, FlatList, Alert, Modal, KeyboardAvoidingView } from "react-native";
...
const REPORT_TYPES = [
    ...
    { id: "safety", label: "Safety Concern", icon: "shield-checkmark", color: Colors.warning },
    { id: "complaint", label: "Complaint", icon: "chatbubble-ellipses", color: Colors.textSecondary },
];
export default function SafetyScreen() {
    ...
    const renderReport = useCallback(({ item }) => (<View style={styles.reportCard}>
      ...
      {item.aiResponse && (<View style={styles.aiBox}>
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]}>
      ...
      {isLoading ? (<View style={styles.center}>
        ...
        </View>) : !reports || (Array.isArray(reports) && reports.length === 0) ? (<View style={styles.center}>
        ...
        </View>) : (<FlatList data={Array.isArray(reports) ? reports : []} keyExtractor={(item) => item.id} renderItem={renderReport} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}/>)}
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setShowReport(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 24) }]} onStartShouldSetResponder={() => true}>
            ...
            <View style={styles.typeRow}>
              {REPORT_TYPES.map((rt) => (<Pressable key={rt.id} style={[styles.typeChip, reportType === rt.id && { borderColor: rt.color, backgroundColor: `${rt.color}15` }]} onPress={() => setReportType(rt.id)}>
            ...
            <TextInput style={styles.descInput} placeholder="Describe what happened..." placeholderTextColor={Colors.textMuted} value={description} onChangeText={setDescription} multiline numberOfLines={4} textAlignVertical="top"/>

            {aiResponse ? (<View style={styles.aiResponseBox}>
              ...
              </View>) : (<Pressable style={({ pressed }) => [styles.submitBtn, pressed && { opacity: 0.9 }, submitMutation.isPending && { opacity: 0.7 }]} onPress={() => {
                if (!description.trim()) {
                ...
                {submitMutation.isPending ? (<ActivityIndicator color={Colors.primary}/>) : (<Text style={styles.submitBtnText}>Submit Report</Text>)}
              </Pressable>)}
          </View>
        </Pressable>
