...
import { View, Text, StyleSheet, Pressable, ScrollView, ActivityIndicator, Alert, Platform, Modal, TextInput, } from "react-native";
...
function RandIcon({ size = 20, color = "#fff" }) {
...
export default function ClientWalletScreen() {
    ...
    const paystackPopup = useRef(null);
    const loadData = useCallback(async () => {
        try {
            const [cardsRes, txRes] = await Promise.all([
                apiRequest("GET", "/api/payments/cards").catch((e) => {
                    if (e?.message?.startsWith("401"))
                        throw e;
                    return null;
                }),
                apiRequest("GET", "/api/wallet/transactions").catch((e) => {
                    if (e?.message?.startsWith("401"))
                        throw e;
                    return null;
                }),
    ...
    async function verifyPaystackPayment(reference, successMsg, silentOnFail = false) {
    ...
    async function openPaystackInApp(authorizationUrl, reference, successMsg) {
        paystackRef.current = reference;
        if (Platform.OS === "web") {
            // ── Web: open a popup window, poll for close, auto-verify when closed ──
            const popup = window.open(authorizationUrl, "paystack-checkout", "width=500,height=700,scrollbars=yes,resizable=yes,left=200,top=80");
            paystackPopup.current = popup;
            if (!popup) {
                Alert.alert("Popup Blocked", "Please allow popups for this site, then tap 'Add Card' again to continue.");
                paystackRef.current = null;
                return;
            }
            ...
            try {
                ...
                if (result.url) {
                    try {
                        const urlRef = new URL(result.url).searchParams.get("reference");
                        if (urlRef)
                            resolvedRef = urlRef;
                    }
                    catch { }
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 100 }}>
        ...
        <View style={styles.balanceCard}>
          ...
          <Pressable style={({ pressed }) => [styles.topupBtn, pressed && { opacity: 0.85 }]} onPress={() => setShowTopup(true)}>
        ...
        <View style={styles.section}>
          ...
          {/* Wallet balance row */}
          {balance > 0 && (<View style={styles.paymentRow}>
          ...
          {/* Saved cards */}
          {loading ? (<ActivityIndicator color={Colors.white} style={{ marginVertical: 20 }}/>) : cards.length === 0 ? (<Pressable style={styles.addCardRow} onPress={() => setShowAddCard(true)}>
              ...
              <Ionicons name="chevron-forward" size={16} color={Colors.textMuted}/>
            </Pressable>) : (cards.map(card => (<View key={card.id} style={[styles.paymentRow, card.isDefault && styles.paymentRowDefault]}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  {card.isDefault ? (<View style={styles.defaultBadge}>
                      <Text style={styles.defaultBadgeText}>Default</Text>
                    </View>) : (<Pressable onPress={() => setDefaultCard(card.id)} hitSlop={8}>
        ...
        <View style={styles.section}>
          ...
          {transactions.length === 0 ? (<View style={styles.emptyTx}>
              <Text style={styles.emptyTxIcon}>📋</Text>
              <Text style={styles.emptyTxText}>No transactions yet</Text>
            </View>) : (transactions.map(tx => (<View key={tx.id} style={styles.txRow}>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setShowTopup(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + 24 }]}>
            ...
            <View style={styles.amountRow}>
              {["50", "100", "200", "500"].map(amt => (<Pressable key={amt} style={[styles.amountChip, topupAmount === amt && styles.amountChipActive]} onPress={() => setTopupAmount(amt)}>
            ...
            <View style={styles.customAmountWrap}>
              ...
              <TextInput style={styles.customAmountInput} value={topupAmount} onChangeText={setTopupAmount} keyboardType="numeric" placeholder="Enter amount" placeholderTextColor={Colors.textMuted}/>
            </View>
            ...
            <Pressable style={[styles.payBtn, topupLoading && { opacity: 0.7 }]} onPress={handleTopup} disabled={topupLoading}>
              {topupLoading
      ...
      {paystackVerifying && (<Modal visible transparent animationType="fade">
      ...
      <Modal visible={showAddCard} transparent animationType="slide" onRequestClose={() => setShowAddCard(false)}>
        ...
        ].map((step, i) => (<View key={i} style={styles.addCardStep}>
                  <Ionicons name={step.icon} size={16} color={Colors.accent}/>
                  <Text style={styles.addCardStepText}>{step.text}</Text>
                </View>))}
            </View>

            <Pressable style={[styles.payBtn, addCardLoading && { opacity: 0.7 }]} onPress={handleAddCard} disabled={addCardLoading}>
              {addCardLoading
            ? <ActivityIndicator color={Colors.primary}/>
            : (<View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
