...
export default function ProfileScreen() {
    ...
    return (<ScrollView style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.profileCard}>
        ...
        {user?.createdAt && (<Text style={styles.profilePhone}>Member since {new Date(user.createdAt).getFullYear()}</Text>)}
        <View style={styles.ratingRow}>
          <Ionicons name="star" size={14} color={Colors.warning}/>
          {profileLoading ? (<ActivityIndicator size="small" color={Colors.warning}/>) : (<Text style={styles.ratingText}>
      ...
      {recentReviews.length > 0 && (<View style={styles.reviewsCard}>
          <Text style={styles.reviewsTitle}>Recent Ratings</Text>
          {recentReviews.map((review) => (<View key={review.id} style={styles.reviewItem}>
