...
const DRIVER_DOCS = [
    { id: "pdrp_certificate", label: "PDRP Certificate", icon: "ribbon-outline", hint: "Professional Driving Permit (e-hailing category)" },
    { id: "drivers_license", label: "Valid Driver's License", icon: "car-outline", hint: "Front and back of your driver's license" },
    { id: "driver_evaluation", label: "Driver Evaluation", icon: "clipboard-outline", hint: "Official driver evaluation / assessment certificate" },
    { id: "criminal_background_check", label: "Criminal Background Check", icon: "shield-checkmark-outline", hint: "Police clearance certificate (not older than 6 months)" },
];
const CAR_DOCS = [
    { id: "double_license_disk", label: "Double License Disk", icon: "disc-outline", hint: "Both license disks displayed in the vehicle" },
    { id: "passenger_liability_insurance", label: "Passenger Liability Insurance", icon: "umbrella-outline", hint: "Valid passenger liability insurance certificate" },
    { id: "dekra_report", label: "Dekra Report", icon: "document-text-outline", hint: "Current Dekra vehicle inspection / roadworthy report" },
];
...
export default function ChauffeurRegisterScreen() {
    ...
    // Document uploads
    const [documents, setDocuments] = useState(Object.fromEntries(ALL_DOCS.map(d => [d.id, null])));
    // Driver photo
    const [driverPhoto, setDriverPhoto] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    async function pickImage(docId, useCamera = false) {
        try {
    ...
    function promptPhotoSource(docId) {
        if (Platform.OS === "web") {
            pickImage(docId);
            return;
        }
    ...
    async function handleSubmit() {
        if (!driverPhoto) {
            setError("Please upload or take your driver profile photo.");
            return;
        }
        if (!user)
            return;
        ...
        try {
            ...
            const failedUploads = [];
            const uploadOne = async (docId, uri) => {
                let publicUrl = "";
                try {
            ...
            if (failedUploads.length > 0) {
                Alert.alert("Some documents failed to upload", `The following could not be uploaded to storage and may need to be re-submitted: ${failedUploads.join(", ")}. Your application has been saved — please contact support if this keeps happening.`);
            }
    ...
    const StepIndicator = ({ current }) => (<View style={styles.stepIndicator}>
      {[1, 2, 3].map((n, i) => (<React.Fragment key={n}>
    ...
    if (step === "vehicle") {
        return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 20) }]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.chipRow}>
                {VEHICLE_CATEGORIES.map((vc) => (<Pressable key={vc.id} style={[styles.chip, vehicleType === vc.id && styles.chipActive]} onPress={() => setVehicleType(vc.id)}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.colorRow}>
                {CAR_COLORS.map((c) => (<Pressable key={c} style={[styles.colorChip, carColor === c && styles.colorChipActive]} onPress={() => setCarColor(c)}>
          ...
          <Pressable style={({ pressed }) => [styles.submitBtn, pressed && { opacity: 0.9 }]} onPress={() => { if (validateVehicle())
            setStep("documents"); }}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.form}>
            {DRIVER_DOCS.map((doc) => {
                ...
                return (<Pressable key={doc.id} style={[styles.docRow, uploaded && styles.docRowUploaded]} onPress={() => promptPhotoSource(doc.id)}>
          ...
          <Pressable style={({ pressed }) => [styles.submitBtn, pressed && { opacity: 0.9 }]} onPress={() => { if (validateDocuments())
            setStep("photo"); }}>
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 20) }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.photoPreviewWrap}>
          {driverPhoto ? (<Image source={{ uri: driverPhoto.uri }} style={styles.photoPreview}/>) : (<View style={styles.photoPlaceholder}>
        ...
        ].map((tip, i) => (<View key={i} style={styles.photoTipRow}>
              <Ionicons name={tip.icon} size={16} color={Colors.accent === "#2A2A2A" ? "#888" : Colors.accent}/>
              <Text style={styles.photoTipText}>{tip.text}</Text>
            </View>))}
        </View>

        <View style={styles.photoActions}>
          {Platform.OS !== "web" && (<Pressable style={styles.photoActionBtn} onPress={() => pickImage("driver_photo", true)}>
        ...
        <Pressable style={({ pressed }) => [styles.submitBtn, pressed && { opacity: 0.9 }, loading && { opacity: 0.7 }]} onPress={handleSubmit} disabled={loading}>
