...
export default function ChatScreen() {
    ...
    const { user } = useAuth();
    const { rideId, riderName } = useLocalSearchParams();
    ...
    const renderMessage = useCallback(({ item }) => {
        ...
        return (<View style={[styles.messageBubble, isMe ? styles.myMessage : styles.theirMessage]}>
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      ...
      {isLoading ? (<View style={styles.center}>
        ...
        </View>) : (<FlatList data={Array.isArray(messages) ? [...messages].reverse() : []} keyExtractor={(item) => item.id} renderItem={renderMessage} contentContainerStyle={styles.messagesList} showsVerticalScrollIndicator={false} inverted={true} style={styles.list} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive"/>)}

      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "web" ? 84 : 0}>
        <View style={[styles.inputBar, { paddingBottom: Math.max(insets.bottom, Platform.OS === "web" ? 84 : 16) }]}>
          <TextInput style={styles.textInput} placeholder="Type a message..." placeholderTextColor={Colors.textMuted} value={messageText} onChangeText={setMessageText} multiline returnKeyType="send" onSubmitEditing={() => { if (messageText.trim())
        sendMutation.mutate(); }} blurOnSubmit={false}/>
          <Pressable style={[styles.sendBtn, (!messageText.trim() || sendMutation.isPending) && { opacity: 0.4 }]} onPress={() => { if (messageText.trim() && !sendMutation.isPending)
        sendMutation.mutate(); }}>
