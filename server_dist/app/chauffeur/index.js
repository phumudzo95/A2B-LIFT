...
import { View, Text, StyleSheet, Pressable, Platform, ActivityIndicator, Alert, Modal, Animated, Dimensions, ScrollView, TextInput, KeyboardAvoidingView, } from "react-native";
...
import { RIDE_MATCH_RADIUS_KM, getBestAvailablePosition, toLatLng, watchBestPosition, } from "@/lib/location-utils";
...
export default function ChauffeurDashboard() {
    ...
    const [locationInterval, setLocationIntervalId] = useState(null);
    ...
    const [clientRatingRide, setClientRatingRide] = useState(null);
    ...
    const seenRideIdRef = useRef(null);
    const suppressedRideAlertIdRef = useRef(null);
    const clientSummaryCacheRef = useRef({});
    const lastSpokenNavKeyRef = useRef(null);
    ...
    function getClientFirstName(name, fallback = "Client") {
    ...
    function getRouteOptionTitle(index, summary) {
    ...
    function getRouteOptionIcon(index) {
        if (currentRide?.status === "trip_started" && index === 0) {
    ...
    function getClientSelectedRouteIndex(routes, ride) {
        if (!Array.isArray(routes) || routes.length === 0 || !ride || ride.status !== "trip_started")
            return 0;
    ...
    function reorderRoutesForClientSelection(routes, ride) {
        if (!Array.isArray(routes) || routes.length === 0 || !ride || ride.status !== "trip_started")
            return routes;
    ...
    function getRouteAlternatives(routes, fallbackRoute) {
    ...
    function calcRoutePrice(distanceKm) {
        if (!distanceKm || !currentRide)
            return "";
    ...
    function getRideRouteLabel(routeId) {
    ...
    function getRideRouteIcon(routeId) {
        if (routeId === "faster_route")
    ...
    function getRidePaymentLabel(method) {
        if (method === "card")
            return "Card";
        if (method === "wallet")
            return "Wallet";
    ...
    function getRidePaymentIcon(method) {
        if (method === "card")
            return "card-outline";
        if (method === "wallet")
    ...
    function getRideClientFare(ride) {
    ...
    async function enrichRideClientDetails(ride, fallback = "Client") {
        if (!ride)
            return ride;
    ...
    async function buildFallbackClientProfile(clientId) {
        try {
            ...
            const completedTrips = Array.isArray(rides)
                ? rides.filter((ride) => ride.status === "trip_completed").length
                : 0;
    ...
    async function playTripAlert() {
        ...
        try {
            ...
            if (soundRef.current) {
                ...
                const { sound } = await Audio.Sound.createAsync(require("../../assets/trip-alert.wav"), { shouldPlay: false, volume: 1.0, isLooping: true });
                if (tripAlertTokenRef.current !== alertToken || !tripAlertEnabledRef.current) {
    ...
    useEffect(() => {
        const clearRideFromDiscovery = (ride) => {
            if (!ride?.id)
                return;
    ...
    // ─── Push notifications ───────────────────────────────────────────────────
    useEffect(() => {
        if (!chauffeur?.id || Platform.OS === "web" || isExpoGoAndroid)
            return;
        (async () => {
            try {
        ...
        async function fetchAvailable() {
            ...
            try {
                ...
                if (Array.isArray(trips)) {
                    const enrichedTrips = await Promise.all(trips.map((trip) => enrichRideClientDetails(trip, "Client")));
    ...
    useEffect(() => {
        if (!myLocation || navSteps.length === 0)
            return;
    ...
    useEffect(() => {
        if (!currentRide || navSteps.length === 0)
            return;
        const step = navSteps[currentStepIdx];
        ...
        if (lastSpokenNavKeyRef.current === navKey)
            return;
    ...
    async function fetchChauffeurForUser(userId) {
        try {
            const res = await apiRequest("GET", `/api/chauffeurs/user/${userId}`);
            const c = await res.json();
            setChauffeur(c);
            setIsOnline(c.isOnline || false);
            if (typeof c.todayEarnings === "number")
                setTodayEarnings(c.todayEarnings);
    ...
    async function refreshChauffeur(id) {
        try {
    ...
    function publishChauffeurLocation(next) {
        setMyLocation(next);
        if (chauffeur?.id) {
    ...
    async function fetchDriverRoute(destLat, destLng, options) {
        if (!myLocation)
            return false;
        try {
            const res = await apiRequest("GET", `/api/directions?originLat=${myLocation.lat}&originLng=${myLocation.lng}&destLat=${destLat}&destLng=${destLng}`);
            const data = await res.json();
    ...
    function selectRoute(alt, index) {
    ...
    async function acceptTripFromList(trip) {
        if (!chauffeur || acceptingTripId)
            return;
    ...
    if (!chauffeur.isApproved) {
        return (<View style={[styles.pendingContainer, { paddingTop: insets.top + 20 }]}>
    ...
    const clientDisplayName = currentRide?.clientFirstName ||
        (currentRide?.clientName ? String(currentRide.clientName).split(" ")[0] : null) ||
        "Client";
    const rideStatusLabel = currentRide?.status === "chauffeur_assigned" ? `On the way to pick up ${clientDisplayName}` :
    ...
    return (<>
        ...
        <View style={styles.navModal}>
          <View style={[styles.navModalHeader, { paddingTop: insets.top + 16 }]}>
            <View>
              ...
              {currentRide && (<Text style={styles.navModalRouteHint}>{clientRouteLabel} · {clientPaymentLabel}</Text>)}
            </View>
            <Pressable style={styles.navModalClose} onPress={() => setShowNavModal(false)}>
              <Ionicons name="chevron-down" size={22} color={Colors.white}/>
            </Pressable>
          </View>
          {routeAlternatives.length > 0 && (<View style={styles.routeOptionsContainer}>
              <Text style={styles.routeOptionsTitle}>{routeOptionsHeading}</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingHorizontal: 4 }}>
                {routeAlternatives.map((alt, i) => (<Pressable key={i} style={[styles.routeOptionCard, selectedRouteIndex === i && styles.routeOptionCardSelected]} onPress={() => selectRoute(alt, i)}>
                    ...
                    {calcRoutePrice(alt.distanceKm) ? (<Text style={styles.routeOptionPrice}>{calcRoutePrice(alt.distanceKm)}</Text>) : null}
                  </Pressable>))}
              </ScrollView>
            </View>)}
          <View style={{ flex: 1 }}>
            <A2BMap pickupLocation={currentRide ? { lat: parseFloat(currentRide.pickupLat), lng: parseFloat(currentRide.pickupLng) } : myLocation} dropoffLocation={currentRide ? { lat: parseFloat(currentRide.dropoffLat), lng: parseFloat(currentRide.dropoffLng) } : undefined} driverLocation={myLocation} routePolyline={routePolyline} showDriver={true} followDriver={true} loading={!myLocation}/>
          </View>
          {navSteps.length > 0 && (<View style={styles.navStepBox}>
              <View style={styles.navStepRow}>
                <View style={styles.navArrowCircle}>
                  <Ionicons name={navSteps[currentStepIdx]?.maneuver?.includes("left") ? "arrow-back" :
                navSteps[currentStepIdx]?.maneuver?.includes("right") ? "arrow-forward" :
                    navSteps[currentStepIdx]?.maneuver?.includes("uturn") ? "return-down-back" :
                        navSteps[currentStepIdx]?.maneuver?.includes("roundabout") ? "sync" : "arrow-up"} size={28} color={Colors.white}/>
                </View>
                <View style={{ flex: 1 }}>
          ...
          <View style={[styles.navModalFooter, { paddingBottom: insets.bottom + 16 }]}>
            {(currentRide?.status === "chauffeur_assigned" || currentRide?.status === "chauffeur_arriving") && (<Pressable style={styles.actionBtn} onPress={startTripToDestination}>
                <Text style={styles.actionBtnText}>Start Trip — Rider On Board</Text>
              </Pressable>)}
            {currentRide?.status === "trip_started" && (<Pressable style={[styles.actionBtn, styles.completeBtnStyle]} onPress={() => updateRideStatus("trip_completed")}>
      ...
      <View style={StyleSheet.absoluteFill}>
        <A2BMap pickupLocation={myLocation || (currentRide ? { lat: parseFloat(currentRide.pickupLat), lng: parseFloat(currentRide.pickupLng) } : null)} dropoffLocation={currentRide ? { lat: parseFloat(currentRide.dropoffLat), lng: parseFloat(currentRide.dropoffLng) } : undefined} driverLocation={myLocation} routePolyline={routePolyline} showDriver={true} followDriver={!!currentRide} loading={!myLocation && isOnline}/>
      </View>

      {/* ─── Online pill (top-left) ─── */}

      {/* ─── Floating turn-by-turn nav bar ─── */}
      {currentRide && navSteps.length > 0 && !showNavModal && (<Pressable style={[styles.floatNavBar, { top: insets.top + 60 }]} onPress={() => setShowNavModal(true)}>
          <View style={styles.floatNavArrow}>
            <Ionicons name={navSteps[currentStepIdx]?.maneuver?.includes("left") ? "arrow-back" :
                navSteps[currentStepIdx]?.maneuver?.includes("right") ? "arrow-forward" :
                    navSteps[currentStepIdx]?.maneuver?.includes("uturn") ? "return-down-back" :
                        navSteps[currentStepIdx]?.maneuver?.includes("roundabout") ? "sync" : "arrow-up"} size={24} color={Colors.white}/>
          </View>
      ...
      <Pressable style={[styles.onlinePill, { top: insets.top + 16 }, isOnline ? styles.onlinePillOn : styles.onlinePillOff]} onPress={toggleOnline}>
      ...
      {isOnline && !currentRide && !incomingRide && (<View style={[styles.tripsPanel, { bottom: insets.bottom + 80 }]}>
          ...
          {availableTrips.length > 0 && (<ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tripsScroll} contentContainerStyle={styles.tripsScrollContent}>
              {availableTrips.map((trip) => (<View key={trip.id} style={styles.tripCard}>
                  ...
                  {trip.distKm != null && (<Text style={styles.tripDist}>{trip.distKm.toFixed(1)} km away</Text>)}
                  <Pressable style={[styles.tripAcceptBtn, acceptingTripId === trip.id && { opacity: 0.6 }]} onPress={() => acceptTripFromList(trip)} disabled={!!acceptingTripId}>
      ...
      {/* ─── Active ride card ─── */}
      {currentRide && (<View style={[styles.bottomCard, { bottom: insets.bottom + 80 }]}>
          ...
          <View style={styles.rideInfoPills}>
            ...
            {currentRide.durationMin ? (<View style={styles.rideInfoPill}>
                <Ionicons name="time-outline" size={12} color={Colors.white}/>
                <Text style={styles.rideInfoPillText}>{Math.round(Number(currentRide.durationMin))} min</Text>
              </View>) : null}
          </View>
          {routeAlternatives.length > 0 && (<View style={styles.cardRouteOptionsWrap}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.cardRouteOptionsScroll}>
                {routeAlternatives.map((alt, i) => (<Pressable key={`${alt.polyline}-${i}`} style={[styles.cardRouteOptionChip, selectedRouteIndex === i && styles.cardRouteOptionChipSelected]} onPress={() => selectRoute(alt, i)}>
                    ...
                    {calcRoutePrice(alt.distanceKm) ? (<Text style={[styles.cardRouteOptionPrice, selectedRouteIndex === i && styles.cardRouteOptionTitleSelected]}>
      ...
      <Modal visible={showClientProfile} transparent animationType="slide" onRequestClose={() => setShowClientProfile(false)}>
        <View style={styles.profileModalOverlay}>
          <View style={styles.profileModalCard}>
            ...
            {clientProfileLoading ? (<View style={styles.profileLoadingWrap}>
                <ActivityIndicator size="large" color={Colors.accent}/>
              </View>) : clientProfile ? (<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.profileScrollContent}>
                ...
                <View style={styles.profileStatsRow}>
                  <View style={styles.profileStatBox}>
                    ...
                    <View style={styles.profileStarsRow}>
                      {[1, 2, 3, 4, 5].map((star) => (<Ionicons key={star} name={star <= Math.round(clientProfile.clientRating ?? 0) ? "star" : "star-outline"} size={11} color={Colors.warning}/>))}
                    </View>
                ...
                {clientProfile.totalRatings > 0 && (<View style={styles.profileDistribution}>
                ...
                {clientProfile.ratings.length > 0 ? (<View style={styles.profileReviews}>
                    <Text style={styles.profileSectionTitle}>Recent Reviews</Text>
                    {clientProfile.ratings.map((review) => (<View key={review.id} style={styles.reviewCard}>
                        <View style={styles.reviewHeader}>
                          ...
                          <View style={styles.reviewStars}>
                            {[1, 2, 3, 4, 5].map((star) => (<Ionicons key={star} name={star <= review.rating ? "star" : "star-outline"} size={12} color={Colors.warning}/>))}
                          </View>
                        </View>
      ...
      <Modal visible={showClientRating} transparent animationType="fade" onRequestClose={closeClientRating}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 16 : 0} style={styles.ratingModalOverlay}>
          <ScrollView bounces={false} contentContainerStyle={styles.ratingModalScrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            <View style={styles.ratingModalCard}>
              ...
              <View style={styles.ratingStarsRow}>
                {[1, 2, 3, 4, 5].map((star) => (<Pressable key={star} onPress={() => setClientRating(star)} hitSlop={8}>
                    <Ionicons name={star <= clientRating ? "star" : "star-outline"} size={34} color={Colors.warning}/>
                  </Pressable>))}
              </View>

              <TextInput value={clientRatingComment} onChangeText={setClientRatingComment} placeholder="Optional feedback" placeholderTextColor={Colors.textMuted} multiline style={styles.ratingCommentInput}/>
              ...
              <View style={styles.ratingActionsRow}>
                ...
                <Pressable style={[styles.ratingPrimaryBtn, (clientRating === 0 || submittingClientRating) && { opacity: 0.6 }]} onPress={submitClientRating} disabled={clientRating === 0 || submittingClientRating}>
                  {submittingClientRating ? (<ActivityIndicator size="small" color={Colors.primary}/>) : (<Text style={styles.ratingPrimaryBtnText}>Submit</Text>)}
                </Pressable>
              </View>
            </View>
          </ScrollView>
      ...
      {menuItems.map((item, i) => {
            ...
            return (<Animated.View key={item.label} style={[styles.menuItem, { bottom: insets.bottom + 16, opacity, transform: [{ translateY }] }]}>
