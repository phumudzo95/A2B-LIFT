...
import { View, Text, StyleSheet, Pressable, ScrollView, ActivityIndicator, Platform, RefreshControl, Alert, } from "react-native";
...
export default function ChauffeurNotificationsScreen() {
    ...
    function confirmClearAll() {
        if (notifications.length === 0)
            return;
        Alert.alert("Clear All Notifications", "This will permanently delete all your notifications. Are you sure?", [
    ...
    if (loading) {
        return (<View style={[styles.container, { paddingTop: insets.top }, styles.center]}>
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      <View style={styles.header}>
        ...
        <Pressable onPress={confirmClearAll} style={[styles.clearBtn, notifications.length === 0 && { opacity: 0.3 }]} disabled={notifications.length === 0}>
          <Text style={styles.clearBtnText}>Clear All</Text>
        </Pressable>
      </View>

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadNotifications(); }} tintColor={Colors.white}/>}>
        {notifications.length === 0 ? (<View style={styles.empty}>
          ...
          </View>) : (notifications.map(n => (<Pressable key={n.id} style={[styles.notifCard, !n.isRead && styles.notifUnread]} onPress={() => !n.isRead && markAsRead(n.id)}>
