...
import { View, Text, StyleSheet, Pressable, ScrollView, ActivityIndicator, Alert, Modal, TextInput, Platform, KeyboardAvoidingView, } from "react-native";
...
export default function ChauffeurWalletScreen() {
    ...
    const [banks, setBanks] = useState(SA_BANKS);
    ...
    async function handleWithdraw() {
        ...
        if (isNaN(amount) || amount < 50) {
            Alert.alert("Minimum withdrawal is R50");
            return;
        }
        if (!selectedBank) {
        ...
        if (!accountName.trim()) {
            Alert.alert("Please enter the account holder name");
            return;
        }
        ...
        try {
            ...
            loadData();
            Alert.alert("✅ Withdrawal Requested", data.message || "Your funds will arrive within 24 hours.", [{ text: "OK" }]);
        }
        catch (e) {
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 100 }}>
        ...
        <View style={styles.earningsCard}>
          ...
          <Pressable style={[styles.withdrawBtn, earnings_total < 50 && styles.withdrawBtnDisabled]} onPress={() => setShowWithdraw(true)} disabled={earnings_total < 50}>
          ...
          {earnings_total < 50 && (<View style={styles.minNoteRow}>
        ...
        <View style={styles.tabs}>
          <Pressable style={[styles.tab, selectedTab === "earnings" && styles.tabActive]} onPress={() => setSelectedTab("earnings")}>
            <Text style={[styles.tabText, selectedTab === "earnings" && styles.tabTextActive]}>Earnings</Text>
          </Pressable>
          <Pressable style={[styles.tab, selectedTab === "withdrawals" && styles.tabActive]} onPress={() => setSelectedTab("withdrawals")}>
            <Text style={[styles.tabText, selectedTab === "withdrawals" && styles.tabTextActive]}>Withdrawals</Text>
          </Pressable>
        </View>

        {selectedTab === "earnings" && (<View style={styles.listSection}>
            {earnings.length === 0 ? (<View style={styles.empty}>
                ...
                return (<View key={e.id} style={styles.listRow}>
        ...
        {selectedTab === "withdrawals" && (<View style={styles.listSection}>
            {withdrawals.length === 0 ? (<View style={styles.empty}>
                <Text style={styles.emptyIcon}>🏦</Text>
                <Text style={styles.emptyText}>No withdrawals yet</Text>
                <Text style={styles.emptySub}>Your withdrawal history will appear here</Text>
              </View>) : (withdrawals.map(w => (<View key={w.id} style={styles.listRow}>
                  <View style={styles.listIcon}>
      ...
      <Modal visible={showWithdraw} transparent animationType="slide" onRequestClose={() => { if (showBankPicker)
        ...
        <>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                  {banks.map((bank, idx) => (<Pressable key={`${bank.code}-${idx}`} style={[styles.bankRow, selectedBank?.code === bank.code && selectedBank?.name === bank.name && styles.bankRowSelected]} onPress={() => { setSelectedBank(bank); setShowBankPicker(false); }}>
                      <Text style={styles.bankName}>{bank.name}</Text>
                      {selectedBank?.code === bank.code && selectedBank?.name === bank.name && (<Ionicons name="checkmark-circle" size={18} color={Colors.success}/>)}
                    </Pressable>))}
                </ScrollView>
              </>) : (
        /* ── Withdrawal form ── */
                ...
                <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                  <View style={styles.fieldGroup}>
                    ...
                    <View style={styles.fieldWrap}>
                      ...
                      <TextInput style={styles.fieldInput} value={withdrawAmount} onChangeText={setWithdrawAmount} keyboardType="numeric" placeholder="0.00" placeholderTextColor={Colors.textMuted}/>
                    </View>
                  </View>
                  ...
                  <View style={styles.fieldGroup}>
                    ...
                    <View style={styles.fieldWrap}>
                      <TextInput style={[styles.fieldInput, { paddingLeft: 0 }]} value={accountNumber} onChangeText={setAccountNumber} keyboardType="numeric" placeholder="Enter account number" placeholderTextColor={Colors.textMuted}/>
                    </View>
                  </View>
                  ...
                  <View style={styles.fieldGroup}>
                    ...
                    <View style={styles.fieldWrap}>
                      <TextInput style={[styles.fieldInput, { paddingLeft: 0 }]} value={accountName} onChangeText={setAccountName} placeholder="As it appears on your bank account" placeholderTextColor={Colors.textMuted}/>
                    </View>
                  </View>
                  ...
                  <View style={styles.modalActions}>
                    ...
                    <Pressable style={[styles.confirmBtn, withdrawLoading && { opacity: 0.7 }]} onPress={handleWithdraw} disabled={withdrawLoading}>
                      {withdrawLoading
