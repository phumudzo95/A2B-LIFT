...
import { View, Text, StyleSheet, FlatList, ActivityIndicator, Platform, RefreshControl, Pressable, ScrollView, } from "react-native";
...
function getGrossFare(ride) {
    return Number(ride?.actualFare || ride?.price || 0);
}
function getDriverFare(ride) {
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.detailScroll}>
        ...
        <View style={styles.detailCard}>
          ...
          {ride.completedAt && (<DetailRow icon="checkmark-circle-outline" label="Completed At" value={`${formatDate(ride.completedAt)} ${formatTime(ride.completedAt)}`}/>)}
          {ride.distanceKm && (<DetailRow icon="navigate-outline" label="Distance" value={`${Number(ride.distanceKm).toFixed(1)} km`}/>)}
          {ride.vehicleType && (<DetailRow icon="car-sport-outline" label="Vehicle Type" value={ride.vehicleType.replace(/_/g, " ")}/>)}
        </View>
...
export default function ChauffeurRidesScreen() {
    ...
    useEffect(() => {
        AsyncStorage.getItem("a2b_chauffeur").then((stored) => {
            if (stored)
                setChauffeurId(JSON.parse(stored).id);
        });
    }, []);
    ...
    const renderRide = useCallback(({ item }) => (<Pressable style={({ pressed }) => [styles.rideCard, pressed && { opacity: 0.8 }]} onPress={() => setSelectedRide(item)}>
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]}>
      ...
      {isLoading ? (<View style={styles.center}>
        ...
        </View>) : !rides || (Array.isArray(rides) && rides.length === 0) ? (<View style={styles.center}>
        ...
        </View>) : (<FlatList data={Array.isArray(rides) ? rides : []} keyExtractor={(item) => item.id} renderItem={renderRide} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={false} onRefresh={refetch} tintColor={Colors.white}/>}/>)}
    </View>);
}
