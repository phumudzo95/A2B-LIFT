...
import { View, Text, StyleSheet, Pressable, Platform, ScrollView, ActivityIndicator, TextInput, Alert, RefreshControl, Modal, KeyboardAvoidingView, } from "react-native";
...
export default function EarningsScreen() {
    ...
    const withdrawMutation = useMutation({
        mutationFn: async () => {
            ...
            if (amt > totalEarnings)
                throw new Error(`You only have R${totalEarnings.toFixed(2)} available to withdraw`);
            if (!selectedBank)
                throw new Error("Select a bank");
            if (!accountNumber.trim())
                throw new Error("Enter your account number");
            if (!accountName.trim())
    ...
    const cardEarnings = earningsList.filter((e) => e.type === "card");
    const totalEarnings = cardEarnings.reduce((sum, e) => sum + (e.amount || 0), 0);
    const totalCommission = cardEarnings.reduce((sum, e) => sum + (e.commission || 0), 0);
    const withdrawalsList = Array.isArray(withdrawals) ? withdrawals : [];
    const banksList = Array.isArray(banksData) ? banksData : [];
    return (<ScrollView style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={false} onRefresh={refetch} tintColor={Colors.white}/>}>
      ...
      <View style={styles.totalCard}>
        ...
        <Pressable style={({ pressed }) => [styles.withdrawBtn, pressed && { opacity: 0.9 }]} onPress={() => setShowWithdraw(true)}>
      ...
      {withdrawalsList.length > 0 && (<>
          ...
          {withdrawalsList.map((w) => (<View key={w.id} style={styles.withdrawalItem}>
      ...
      {earningsList.length > 0 && (<>
          ...
          {earningsList.slice(0, 10).map((e) => (<View key={e.id} style={styles.earningItem}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <View style={[styles.amountRow, parseFloat(withdrawAmount) > totalEarnings && !!withdrawAmount && { borderColor: Colors.error }]}>
              ...
              <TextInput style={styles.amountInput} placeholder="0" placeholderTextColor={Colors.textMuted} value={withdrawAmount} onChangeText={setWithdrawAmount} keyboardType="number-pad"/>
            </View>
            {!!withdrawAmount && parseFloat(withdrawAmount) > totalEarnings && (<Text style={styles.inputError}>
                You only have R{totalEarnings.toFixed(2)} available. Please enter a lower amount.
              </Text>)}

            <Text style={styles.fieldLabel}>Bank</Text>
            <Pressable style={[styles.bankSelector, showBankPicker && { borderColor: Colors.white }]} onPress={() => setShowBankPicker((v) => !v)}>
            ...
            {showBankPicker && (<View style={styles.bankDropdown}>
                {banksLoading ? (<ActivityIndicator color={Colors.white} style={{ paddingVertical: 16 }}/>) : banksList.length === 0 ? (<Text style={styles.bankEmptyText}>No banks found. Try again.</Text>) : (<ScrollView style={{ maxHeight: 200 }} nestedScrollEnabled keyboardShouldPersistTaps="handled">
                    {banksList.map((bank) => (<Pressable key={bank.code} style={({ pressed }) => [styles.bankDropdownItem, pressed && { opacity: 0.7 }]} onPress={() => { setSelectedBank(bank); setShowBankPicker(false); }}>
                        <Text style={styles.bankItemText}>{bank.name}</Text>
                        {selectedBank?.code === bank.code && (<Ionicons name="checkmark" size={16} color={Colors.success}/>)}
                      </Pressable>))}
                  </ScrollView>)}
              </View>)}

            <Text style={styles.fieldLabel}>Account Number</Text>
            <TextInput style={styles.textField} placeholder="Enter account number" placeholderTextColor={Colors.textMuted} value={accountNumber} onChangeText={setAccountNumber} keyboardType="number-pad"/>

            <Text style={styles.fieldLabel}>Account Holder Name</Text>
            <TextInput style={styles.textField} placeholder="Name as on bank account" placeholderTextColor={Colors.textMuted} value={accountName} onChangeText={setAccountName}/>

            <Pressable style={({ pressed }) => [styles.submitBtn, pressed && { opacity: 0.85 }, withdrawMutation.isPending && { opacity: 0.6 }]} onPress={() => withdrawMutation.mutate()} disabled={withdrawMutation.isPending}>
...
const styles = StyleSheet.create({
    ...
    totalLabel: { fontSize: 13, fontFamily: "Inter_500Medium", color: Colors.textSecondary, textTransform: "uppercase", letterSpacing: 1 },
    ...
    fieldLabel: { fontSize: 12, fontFamily: "Inter_600SemiBold", color: Colors.textSecondary, textTransform: "uppercase", letterSpacing: 0.8 },
    ...
    bankEmptyText: { fontSize: 13, fontFamily: "Inter_400Regular", color: Colors.textMuted, textAlign: "center", paddingVertical: 16 },
