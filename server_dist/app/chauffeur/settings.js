...
export default function ChauffeurSettingsScreen() {
    ...
    async function loadDriverProfile(chauffeurId) {
        if (!chauffeurId)
            return;
        try {
    ...
    async function pickAndUploadDocument(type) {
        try {
    ...
    function getDocumentStatus(type) {
        if (!userDocuments || !Array.isArray(userDocuments))
            return "missing";
        const doc = userDocuments.find((d) => d.type === type || d.documentType === type);
        if (!doc)
            return "missing";
    ...
    return (<ScrollView style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 16) }]} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      <Text style={styles.title}>Settings</Text>

      <View style={styles.profileCard}>
        <View style={styles.avatar}>
          {(chauffeur?.profilePhoto || user?.profilePhoto) ? (<Image source={{ uri: chauffeur?.profilePhoto || user?.profilePhoto }} style={styles.avatarImg}/>) : (<Ionicons name="person" size={28} color={Colors.white}/>)}
        </View>
        ...
        <View style={styles.ratingRow}>
          ...
          {driverProfileLoading ? (<ActivityIndicator size="small" color={Colors.warning}/>) : (<Text style={styles.ratingText}>
      ...
      {!!driverProfile?.ratings?.length && (<View style={styles.reviewsCard}>
          ...
          {ratingPreview.map((review) => (<View key={review.id} style={styles.reviewItem}>
      ...
      <View style={styles.menuGroup}>
        <Pressable style={({ pressed }) => [styles.menuItem, pressed && { opacity: 0.7 }]} onPress={() => router.replace("/role-select")}>
          <View style={styles.menuIconCircle}>
      ...
      <Modal visible={showVehicle} transparent animationType="slide" onRequestClose={() => setShowVehicle(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowVehicle(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]} onStartShouldSetResponder={() => true}>
            ...
            {chauffeur ? (<View style={styles.detailsList}>
                {chauffeur.carMake && (<View style={styles.detailRow}>
              ...
              </View>) : (<Text style={styles.noDataText}>Loading vehicle information...</Text>)}
          </View>
        </Pressable>
      </Modal>
      ...
      <Modal visible={showDocuments} transparent animationType="slide" onRequestClose={() => setShowDocuments(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowDocuments(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]} onStartShouldSetResponder={() => true}>
            ...
            const docData = Array.isArray(userDocuments) ? userDocuments.find((d) => d.type === type || d.documentType === type) : null;
            return (<View key={type} style={styles.docItem}>
                    <Pressable style={[styles.docItemInner, isUploading && { opacity: 0.6 }]} onPress={() => !isUploading && pickAndUploadDocument(type)} disabled={isUploading}>
                      ...
                      {isUploading ? (<ActivityIndicator size="small" color={Colors.white}/>) : (<View style={[styles.docBadge, badge.bg]}>
                          <Text style={styles.docBadgeText}>{badge.text}</Text>
                        </View>)}
                      {status === "missing" && (<Ionicons name="cloud-upload-outline" size={18} color={Colors.textMuted} style={{ marginLeft: 8 }}/>)}
                    </Pressable>
                    {docData?.url && (<View style={styles.docPreview}>
      ...
      <Modal visible={showApplicationStatus} transparent animationType="slide" onRequestClose={() => setShowApplicationStatus(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowApplicationStatus(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]} onStartShouldSetResponder={() => true}>
            ...
            {application ? (<View style={styles.appStatusContainer}>
                ...
                application.status === "approved" ? styles.statusApprovedLarge :
                  ...
                  <Ionicons name={application.status === "approved" ? "checkmark-circle" : application.status === "rejected" ? "close-circle" : "time"} size={32} color={Colors.white}/>
          ...
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]} onStartShouldSetResponder={() => true}>
            ...
            {driverProfile ? (<>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.ratingsList}>
                  {driverProfile.ratings.map((review) => (<View key={review.id} style={styles.reviewItem}>
                      <View style={styles.reviewHeader}>
                        <Text style={styles.reviewName}>{review.reviewerName}</Text>
                        <Text style={styles.reviewMeta}>{review.rating.toFixed(1)} ★</Text>
                      </View>
...
const styles = StyleSheet.create({
    ...
    avatar: { width: 64, height: 64, borderRadius: 32, backgroundColor: Colors.accent, alignItems: "center", justifyContent: "center", marginBottom: 6, overflow: "hidden" },
    ...
    docItem: { backgroundColor: Colors.surface, borderRadius: 12, borderWidth: 1, borderColor: Colors.border, marginBottom: 8, overflow: "hidden" },
