...
export default function RoleSelectScreen() {
    ...
    if (!user)
        return null;
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 40) }]}>
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16) }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.cardsContainer}>
          <Animated.View entering={FadeInDown.duration(600).delay(200)}>
            <Pressable style={({ pressed }) => [styles.roleCard, pressed && { opacity: 0.9, transform: [{ scale: 0.98 }] }]} onPress={() => { AsyncStorage.setItem("a2b_last_mode", "client"); router.push("/client"); }}>
          ...
          <Animated.View entering={FadeInDown.duration(600).delay(400)}>
            <Pressable style={({ pressed }) => [styles.roleCard, pressed && { opacity: 0.9, transform: [{ scale: 0.98 }] }]} onPress={() => { AsyncStorage.setItem("a2b_last_mode", "chauffeur"); router.push("/chauffeur"); }}>
