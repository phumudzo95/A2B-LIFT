...
async function configureExpoAndLanding(app) {
    const isRailwayRuntime = Boolean(process.env.RAILWAY_PUBLIC_DOMAIN ||
    ...
    if (staticBuildExists) {
        ...
        app.use((req, res, next) => {
            if (req.path.startsWith("/api"))
                return next();
            if (req.path === "/admin" || req.path === "/a2b-admin" || req.path.startsWith("/admin/") || req.path.startsWith("/a2b-admin/"))
                return next(); // handled above
            if (req.path.startsWith("/socket.io"))
                return next(); // let Socket.IO handle this
