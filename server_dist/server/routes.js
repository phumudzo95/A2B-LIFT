...
import { users, livenessSessions } from "../shared/schema";
...
import { authOptional, requireAuth, requireRole } from "./auth-middleware";
...
function hasFreshChauffeurLocation(chauffeur) {
    if (chauffeur.lat == null || chauffeur.lng == null)
        return false;
    if (!chauffeur.locationUpdatedAt)
        return true;
...
export async function registerRoutes(app) {
    ...
    function getUserFirstName(user, fallback = "Rider") {
        ...
        for (const candidate of candidates) {
            ...
            if (["a2b", "client", "rider", "user", "oauth"].includes(lowered))
                continue;
    ...
    function setDirectionsCacheEntry(cacheKey, payload) {
        ...
        if (directionsCache.size <= DIRECTIONS_CACHE_MAX_ENTRIES)
            return;
    ...
    app.post("/api/chauffeurs", authOptional, async (req, res) => {
        try {
    ...
    app.get("/api/chauffeurs/user/:userId", async (req, res) => {
        try {
    ...
    app.put("/api/chauffeurs/:id/push-token", requireAuth, async (req, res) => {
        try {
    ...
    app.get("/api/chauffeurs/:id", async (req, res) => {
        try {
            ...
            const computedRating = ratings.length > 0
                ...
                : null;
            const cardEarningsTotal = earningsList
            ...
            const todayCardEarnings = earningsList
                .filter((e) => e.createdAt && new Date(e.createdAt) >= todayStart && (e.type === "card" || e.type === "wallet"))
                .reduce((s, e) => s + (e.amount || 0), 0);
            ...
            const todayCashFares = chauffeurRides
                .filter((r) => r.status === "trip_completed" && r.paymentMethod === "cash" && r.completedAt && new Date(r.completedAt) >= todayStart)
                .reduce((s, r) => s + calculateChauffeurEarnings(r.price || 0).chauffeurEarnings, 0);
    ...
    app.get("/api/chauffeurs/:id/details", async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.id);
            if (!chauffeur)
                return res.status(404).json({ message: "Chauffeur not found" });
    ...
    app.get("/api/chauffeurs/:id/profile", async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.id);
            if (!chauffeur)
                return res.status(404).json({ message: "Chauffeur not found" });
    ...
    app.delete("/api/chauffeurs/:id", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.id);
            if (!chauffeur)
                return res.status(404).json({ message: "Chauffeur not found" });
            ...
            if (chauffeur.userId) {
                ...
                if (app)
                    await storage.deleteDriverApplication(app.id);
            }
    ...
    app.post("/api/chauffeurs/:id/approve", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.id);
            if (!chauffeur)
                return res.status(404).json({ message: "Chauffeur not found" });
    ...
    app.post("/api/chauffeurs/:id/reject", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
    ...
    app.get("/api/chauffeurs/:id/documents", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.id);
            if (!chauffeur)
                return res.status(404).json({ message: "Chauffeur not found" });
    ...
    app.put("/api/chauffeurs/:id/toggle-online", async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.id);
            if (!chauffeur)
                return res.status(404).json({ message: "Chauffeur not found" });
    ...
    app.get("/api/chauffeurs", async (_req, res) => {
        try {
            const allChauffeurs = await storage.getAllChauffeurs();
            // Enrich with user details (name, username, phone)
            const enriched = await Promise.all(allChauffeurs.map(async (c) => {
    ...
    // -----------------------------
    app.get("/api/driver/applications/me", authOptional, async (req, res) => {
    ...
    app.get("/api/admin/driver-applications", requireAuth, requireRole(["admin"]), async (_req, res) => {
        const apps = await storage.getDriverApplications();
        return res.json(apps);
    });
    app.put("/api/admin/driver-applications/:id", requireAuth, requireRole(["admin"]), async (req, res) => {
    ...
    app.delete("/api/admin/driver-applications/:id", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
    ...
    app.post("/api/upload/profile-photo", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
    ...
    app.post("/api/upload-document", authOptional, async (req, res) => {
        try {
    ...
    app.get("/api/driver/documents", authOptional, async (req, res) => {
        const userId = req.auth?.sub || req.query.userId;
        if (!userId)
            return res.status(400).json({ message: "userId required" });
    ...
    app.get("/api/admin/documents", requireAuth, requireRole(["admin"]), async (_req, res) => {
        const docs = await storage.getAllDocuments();
        return res.json(docs);
    });
    app.put("/api/admin/documents/:id", requireAuth, requireRole(["admin"]), async (req, res) => {
    ...
    app.get("/api/pricing/categories", async (_req, res) => {
        return res.json(getVehicleCategories());
    });
    // -----------------------------
    // Liveness (Cash Ride Security)
    // -----------------------------
    app.post("/api/liveness/session", requireAuth, async (req, res) => {
        try {
    ...
    app.post("/api/liveness/verify", requireAuth, async (req, res) => {
        try {
    ...
    app.post("/api/rides", requireAuth, async (req, res) => {
        try {
            ...
            const livenessVerifiedAt = rideData.livenessStatus === "passed" ? new Date() : undefined;
            ...
            if (nearbyChauffeurs.length > 0) {
                ...
                if (pushTokens.length > 0) {
                    sendExpoPushNotification(pushTokens, "🚗 New Ride Request", `Pickup: ${ride.pickupAddress || "Nearby"} — tap to accept`, { rideId: ride.id, type: "ride:new" }, { urgent: true });
                }
            }
            else {
    ...
    app.put("/api/rides/:id/accept", requireAuth, async (req, res) => {
        try {
            ...
            if (!chauffeurId)
                return res.status(400).json({ message: "chauffeurId is required" });
            ...
            if (chauffeur.pushToken) {
                sendExpoPushNotification([chauffeur.pushToken], "🚗 Going to Pick Up", "You've accepted the ride. Head to the pickup location now.", { rideId: updated.id, type: "ride:accepted" });
            }
        ...
        try {
            ...
            if (status === "cancelled" && rideBeforeUpdate) {
                try {
                    ...
                    const cardPayment = payments.find((p) => p.method === "card" && p.status === "paid" && p.paystackReference);
                    if (cardPayment?.paystackReference) {
                        const secret = process.env.PAYSTACK_SECRET_KEY || "";
                        await axios.post("https://api.paystack.co/refund", { transaction: cardPayment.paystackReference }, { headers: { Authorization: `Bearer ${secret}`, "Content-Type": "application/json" } });
                    ...
                    const walletPayment = !cardPayment
                        ? payments.find((p) => p.method === "wallet" && p.status === "paid")
                        : null;
                    ...
                    if (rideBeforeUpdate.chauffeurId) {
                        ...
                        if (chauffeur?.pushToken) {
                            sendExpoPushNotification([chauffeur.pushToken], "Ride Cancelled", "The client has cancelled this trip.");
                        }
                    }
                }
                catch (refundErr) {
            ...
            try {
                if (status === "chauffeur_arriving" && ride.clientId) {
                    ...
                    if (riderUser?.pushToken) {
                        sendExpoPushNotification([riderUser.pushToken], "🚗 Driver Arriving", "Your chauffeur is arriving at your pickup. Please be ready!", { rideId: ride.id, type: "ride:arriving" });
                    }
                }
                    ...
                    const riderUser = await storage.getUser(ride.clientId);
                    if (riderUser?.pushToken) {
                        sendExpoPushNotification([riderUser.pushToken], "🚀 Trip Started", `Your ride is underway to ${ride.dropoffAddress || "your destination"}.`, { rideId: ride.id, type: "ride:started" });
                    }
                }
            }
            catch (notifErr) {
    ...
    app.post("/api/rides/:id/pay", requireAuth, async (req, res) => {
        const ride = await storage.getRide(req.params.id);
        if (!ride)
            return res.status(404).json({ message: "Ride not found" });
        if (ride.clientId !== req.auth.sub)
            return res.status(403).json({ message: "Forbidden" });
    ...
    app.post("/api/rides/:id/rate", requireAuth, async (req, res) => {
        ...
        if (ride.clientId !== req.auth.sub)
            return res.status(403).json({ message: "Forbidden" });
        if (!ride.chauffeurId)
            return res.status(400).json({ message: "Ride has no chauffeur" });
    ...
    app.post("/api/rides/:id/rate-client", requireAuth, async (req, res) => {
        try {
            await ensureClientRatingsTable();
            const { rating, comment } = req.body;
        ...
        `, [ride.id, ride.clientId, ride.chauffeurId, numericRating, comment || null]);
            const averageResult = await pool.query(`SELECT ROUND(AVG(rating)::numeric, 2) AS avg_rating FROM client_ratings WHERE client_id = $1`, [ride.clientId]);
    ...
    app.get("/api/rides/chauffeur/:chauffeurId", async (req, res) => {
        try {
    ...
    app.get("/api/rides/available/:chauffeurId", async (req, res) => {
        try {
            ...
            if (chauffeur.lat && chauffeur.lng) {
                candidates = searching
                    .map((r) => ({
                    ...r,
                    distKm: haversine(Number(chauffeur.lat), Number(chauffeur.lng), parseFloat(r.pickupLat), parseFloat(r.pickupLng)),
                }))
                    .filter((r) => r.distKm <= RIDE_MATCH_RADIUS_KM)
                    .sort((a, b) => a.distKm - b.distKm);
            }
            // Enrich with client first name
            const enriched = await Promise.all(candidates.slice(0, 10).map(async (r) => {
                try {
    ...
    app.get("/api/rides/chauffeur-pending/:chauffeurId", async (req, res) => {
        try {
            const chauffeur = await storage.getChauffeur(req.params.chauffeurId);
            if (!chauffeur || !chauffeur.isOnline || !chauffeur.isApproved) {
    ...
    // -----------------------------
    app.get("/api/earnings/chauffeur/:chauffeurId", async (req, res) => {
        try {
    ...
    app.get("/api/withdrawals/chauffeur/:chauffeurId", async (req, res) => {
        try {
    ...
    app.get("/api/messages/ride/:rideId", async (req, res) => {
        try {
    ...
    app.get("/api/safety-reports/user/:userId", async (req, res) => {
        try {
    ...
    app.post("/api/trip-enquiries", requireAuth, async (req, res) => {
        try {
    ...
    app.get("/api/trip-enquiries", requireAuth, requireRole(["admin"]), async (_req, res) => {
        try {
    ...
    app.post("/api/trip-enquiries/:id/reply", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
    ...
    app.get("/api/admin/liveness-selfies", requireAuth, requireRole(["admin"]), async (_req, res) => {
        try {
            const allRides = await storage.getAllRides();
            ...
            const records = await Promise.all(selfieRides.map(async (ride) => {
    ...
    app.get("/api/admin/stats", requireAuth, requireRole(["admin"]), async (_req, res) => {
        try {
            ...
            const activeRides = allRides.filter((r) => !["trip_completed", "cancelled"].includes(r.status));
    ...
    app.get("/api/payments/webview-callback", (req, res) => {
  ...
  <script>
    ...
    app.post("/api/payments/verify", requireAuth, async (req, res) => {
        try {
            ...
            if (psMsg)
                return res.status(400).json({ message: psMsg });
            return res.status(500).json({ message: "Payment verification failed" });
        }
    });
    // POST /api/payments/charge-card
    app.post("/api/payments/charge-card", requireAuth, async (req, res) => {
        try {
    ...
    app.post("/api/payments/charge-ride", requireAuth, async (req, res) => {
        try {
            const { rideId } = req.body;
            const userId = req.auth.sub;
            const user = await storage.getUser(userId);
    ...
    app.post("/api/payments/pay-wallet", requireAuth, async (req, res) => {
        try {
            const { rideId } = req.body;
            let { amount } = req.body;
            const userId = req.auth.sub;
    ...
    app.get("/api/payments/cards", requireAuth, async (req, res) => {
        try {
    ...
    app.delete("/api/payments/cards/:id", requireAuth, async (req, res) => {
        try {
    ...
    app.put("/api/payments/cards/:id/default", requireAuth, async (req, res) => {
        try {
            const userId = req.auth.sub;
            const cards = await storage.getSavedCardsByUser(userId);
    ...
    app.get("/api/wallet/banks", async (_req, res) => {
        try {
    ...
    app.post("/api/rides/:id/select-route", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post("/api/rides/:id/upload-photo", requireAuth, async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.get("/api/admin/rides/:id/photos", requireAuth, requireRole(["admin"]), async (req, res) => {
        try {
            const ride = await storage.getRide(req.params.id);
            if (!ride)
                return res.status(404).json({ error: "Ride not found" });
            ...
            async function makeSignedUrl(bucket, path) {
                if (!path || !SERVICE_KEY)
                    return null;
            ...
            const livPhotos = await Promise.all(livSessions.map(async (sess) => ({
                id: sess.id,
                status: sess.status,
                score: sess.score,
