...
const REQUIRED_DOCS = [
    { id: "id_document", label: "South African ID / Passport", icon: "card-outline", hint: "Clear photo of your ID document" },
    { id: "drivers_license", label: "Driver's License", icon: "car-outline", hint: "Front and back of your license" },
    { id: "proof_of_address", label: "Proof of Address", icon: "home-outline", hint: "Utility bill or bank statement (not older than 3 months)" },
    { id: "vehicle_registration", label: "Vehicle Registration", icon: "document-outline", hint: "Official registration document (RC1)" },
    { id: "prDP", label: "PrDP Certificate", icon: "ribbon-outline", hint: "Professional Driving Permit (required for e-hailing)" },
];
export default function ChauffeurRegisterScreen() {
    ...
    async function pickDocument(docId) {
        try {
            if (Platform.OS === "web") {
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.form}>
          {REQUIRED_DOCS.map((doc) => {
            ...
            return (<Pressable key={doc.id} style={[styles.docRow, uploaded && styles.docRowUploaded]} onPress={() => pickDocument(doc.id)}>
...
export default function ChauffeurRegisterScreen() {
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 20) }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <Pressable style={({ pressed }) => [styles.submitBtn, pressed && { opacity: 0.9, transform: [{ scale: 0.98 }] }, loading && { opacity: 0.7 }]} onPress={handleSubmit} disabled={loading}>
