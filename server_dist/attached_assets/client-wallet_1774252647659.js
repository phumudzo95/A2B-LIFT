...
import { View, Text, StyleSheet, Pressable, ScrollView, ActivityIndicator, Alert, Linking, Platform, Modal, TextInput, } from "react-native";
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 100 }}>
        ...
        <View style={styles.section}>
          ...
          {/* Saved cards */}
          {loading ? (<ActivityIndicator color={Colors.white} style={{ marginVertical: 20 }}/>) : cards.length === 0 ? (<Pressable style={styles.addCardRow} onPress={() => setShowTopup(true)}>
              <Ionicons name="card-outline" size={20} color={Colors.textMuted}/>
              <Text style={styles.addCardText}>No saved cards · Tap to add one</Text>
            </Pressable>) : (cards.map(card => (<View key={card.id} style={styles.paymentRow}>
                <View style={styles.paymentIcon}>
      ...
      <Modal visible={showTopup} transparent animationType="slide" onRequestClose={() => setShowTopup(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowTopup(false)}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + 24 }]}>
            ...
            <Pressable style={[styles.payBtn, topupLoading && { opacity: 0.7 }]} onPress={handleAddCard} disabled={topupLoading}>
              {topupLoading
