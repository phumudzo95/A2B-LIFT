...
export async function registerRoutes(app) {
    ...
    app.get("/api/geocode", async (req, res) => {
        try {
            ...
            const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}&limit=1`, { headers: { "User-Agent": "A2BLIFT/1.0" } });
    ...
    // -----------------------------
    app.get("/api/driver/applications/me", requireAuth, async (req, res) => {
    ...
    app.get("/api/driver/documents", requireAuth, async (req, res) => {
