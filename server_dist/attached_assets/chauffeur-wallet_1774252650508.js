...
import { View, Text, StyleSheet, Pressable, ScrollView, ActivityIndicator, Alert, Modal, TextInput, Platform, } from "react-native";
    ...
    return (<View style={[styles.container, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 100 }}>
        ...
        <View style={styles.earningsCard}>
          ...
          <Pressable style={[styles.withdrawBtn, earnings_total < 50 && { opacity: 0.5 }]} onPress={() => earnings_total >= 50 ? setShowWithdraw(true) : Alert.alert("Minimum withdrawal is R50")}>
          ...
          {earnings_total < 50 && (<Text style={styles.minNote}>Minimum withdrawal is R50</Text>)}
        </View>
        ...
        {selectedTab === "earnings" && (<View style={styles.listSection}>
            {earnings.length === 0 ? (<View style={styles.empty}>
              ...
              </View>) : (earnings.map(e => (<View key={e.id} style={styles.listRow}>
      ...
      <Modal visible={showBankPicker} transparent animationType="slide" onRequestClose={() => setShowBankPicker(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { paddingBottom: insets.bottom + 24, maxHeight: "70%" }]}>
            ...
            <ScrollView>
              {banks.map(bank => (<Pressable key={bank.code} style={[styles.bankRow, selectedBank?.code === bank.code && styles.bankRowSelected]} onPress={() => { setSelectedBank(bank); setShowBankPicker(false); }}>
                  <Text style={styles.bankName}>{bank.name}</Text>
                  {selectedBank?.code === bank.code && (<Ionicons name="checkmark-circle" size={18} color={Colors.success}/>)}
                </Pressable>))}
            </ScrollView>
          </View>
        </View>
