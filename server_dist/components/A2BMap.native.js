...
const DriverMarker = React.memo(({ latitude, longitude }) => (<Marker coordinate={{ latitude, longitude }} anchor={{ x: 0.5, y: 0.5 }} tracksViewChanges={false} flat={true}>
      <View style={driverMarkerStyle.wrap}>
        <Ionicons name="car-sport" size={20} color="#000"/>
      </View>
    </Marker>), (prev, next) => prev.latitude === next.latitude && prev.longitude === next.longitude);
const driverMarkerStyle = { wrap: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center", shadowColor: "#000", shadowOpacity: 0.25, shadowRadius: 4, shadowOffset: { width: 0, height: 2 }, elevation: 6 } };
...
function computeRegion(coords, extraPadFactor = 0.35) {
...
export default function A2BMap({ pickupLocation, dropoffLocation, driverLocation, nearbyDrivers = [], routePolyline, showDriver = false, followDriver = false, loading = false, etaText, statusText, }) {
    const mapRef = useRef(null);
    const mapReadyRef = useRef(false);
    const lastCenteredPickupRef = useRef(null);
    const lastFollowedDriverRef = useRef(null);
    ...
    const zoomToCoords = useCallback((coords, duration = 700) => {
    ...
    useEffect(() => {
        if (!pickupLocation || !mapRef.current)
            return;
        if (routeCoords.length > 0 || dropoffLocation || followDriver)
            return;
        if (lastCenteredPickupRef.current &&
    ...
    useEffect(() => {
        if (!followDriver || !driverLocation || !mapRef.current)
            return;
        if (lastFollowedDriverRef.current &&
    ...
    return (<View style={styles.container}>
      {(loading || !pickupLocation) && (<View style={styles.locatingOverlay}>
          <ActivityIndicator size="small" color={Colors.white}/>
          <Text style={styles.locatingText}>Locating you...</Text>
        </View>)}
      <MapView ref={mapRef} style={styles.map} provider={Platform.OS === "web" ? undefined : PROVIDER_GOOGLE} customMapStyle={DARK_MAP_STYLE} initialRegion={initialRegionRef.current} onMapReady={handleMapReady} showsUserLocation={true} showsMyLocationButton={false} showsCompass={false} showsTraffic={false} showsBuildings={false} showsIndoors={false} toolbarEnabled={false} mapPadding={{ top: 0, right: 0, bottom: 0, left: 0 }} legalLabelInsets={{ bottom: -999, left: 0, top: 0, right: 0 }} googleMapsApiKey={GOOGLE_MAPS_API_KEY}>
        {pickupLocation && (<Marker coordinate={{ latitude: pickupLocation.lat, longitude: pickupLocation.lng }} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.pickupMarker}>
              <View style={styles.pickupDot}/>
            </View>
          </Marker>)}

        {dropoffLocation && (<Marker coordinate={{ latitude: dropoffLocation.lat, longitude: dropoffLocation.lng }} anchor={{ x: 0.5, y: 1 }}>
        ...
        {!showDriver && nearbyDrivers.map((driver) => (<Marker key={`nearby-${driver.id}`} coordinate={{ latitude: driver.lat, longitude: driver.lng }} anchor={{ x: 0.5, y: 0.5 }} tracksViewChanges={false}>
            <View style={styles.nearbyDriverMarker}>
              <Ionicons name="car-sport" size={16} color="#000"/>
            </View>
          </Marker>))}

        {showDriver && driverLocation && (<DriverMarker latitude={driverLocation.lat} longitude={driverLocation.lng}/>)}

        {routeCoords.length > 0 && (<Polyline coordinates={routeCoords} strokeColor="#FFFFFF" strokeWidth={5} zIndex={10} geodesic={true} lineCap="round" lineJoin="round"/>)}
      </MapView>

      {statusText && (<View style={styles.statusOverlay}>
